    ...
    try:
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            for member in members:
                ...
                member_path = _normalized_relative_path(member.filename)
...
def rewrite_html_paths(deploy_root: Path, public_prefix: str | None = None) -> None:
    """
    Rewrite absolute asset paths in HTML files to deployment-safe paths.
    ...
    """
    import re
    replacement_prefix = public_prefix.rstrip("/") if public_prefix else "."
        ...
        for fname in files:
            ...
            try:
                ...
                def _replace(match: re.Match) -> str:
                    ...
                    return f'{attr}={quote}{replacement_prefix}{path}{quote}'
                ...
                # Ignore non-text HTML-like files; the original upload remains intact.
                continue


def rewrite_css_paths(deploy_root: Path, public_prefix: str) -> None:
    """Rewrite root-relative CSS url() references to the deployment path."""
    import re
    ...
    for root, _, files in os.walk(deploy_root):
        for fname in files:
            ...
            try:
                content = fpath.read_text(encoding="utf-8")
                updated = url_pattern.sub(lambda m: f'url({m.group(1)}{prefix}/{m.group(2)}{m.group(3)})', content)
                if updated != content:
...
def inject_env_config(
    ...
    """
    Inject runtime environment variables into the deployed site.

    Generates a ``__staticdrop_env__.js`` file in *deploy_root* that sets
    ``window.__STATICDROP_ENV__`` with the provided key-value pairs, then
    injects a ``<script>`` tag into every ``index.html`` to load it.

    This allows frontend code to read backend API URLs and other config
    at runtime without rebuilding:

        // In the frontend app:
        const API_URL = window.__STATICDROP_ENV__?.API_URL || '/api';

    If *env* is None or empty, this function does nothing.
    """
    if not env:
        return
    ...
    env_json = json.dumps(env, ensure_ascii=False).replace("<", "\\u003c").replace(">", "\\u003e").replace("&", "\\u0026")
    env_js_content = (
        "/* Auto-generated by StaticDrop - do not edit */\n"
        f"window.__STATICDROP_ENV__ = {env_json};\n"
    )
    env_js_path = deploy_root / "__staticdrop_env__.js"
    env_js_path.write_text(env_js_content, encoding="utf-8")

    # Inject <script> tag into all HTML files
    script_tag = f'<script src="{public_path}"></script>'
    ...
    for root, _, files in os.walk(deploy_root):
        for fname in files:
            ...
            try:
                ...
                if "__staticdrop_env__" in content:
                    continue
                # Inject right after <head> or at the beginning of the file
...
def current_deployments_size() -> int:
    """Return the total bytes currently stored in deployed sites."""
    if not config.DEPLOYMENTS_DIR.exists():
        return 0
...
def storage_error(incoming_size: int) -> str | None:
    ...
    if current_deployments_size() + incoming_size > config.MAX_STORAGE_SIZE:
        return f"Deployed storage exceeds maximum of {config.MAX_STORAGE_SIZE} bytes"
    try:
        if shutil.disk_usage(config.DATA_DIR).free < incoming_size + config.MIN_FREE_SPACE:
            return "Not enough free disk space for this deployment"
    except OSError:
        # The deployment directory may be on a virtual filesystem without usage stats.
        pass
    return None


def _normalized_relative_path(relative_path: str) -> Path:
    """Normalize browser/zip paths consistently on all host platforms."""
...
async def safe_write_files(
    ...
    for content, relative_path in files:
        ...
        normalized_path = _normalized_relative_path(relative_path)
...
async def safe_write_uploaded_files(
    files: list["UploadFile"],
    dest_dir: Path,
) -> tuple[int, int, str | None]:
    """Stream folder uploads to disk while enforcing all upload quotas."""
    dest_dir.mkdir(parents=True, exist_ok=True)
    file_count = 0
    total_size = 0
    ...
    if len(files) > config.MAX_FILE_COUNT:
        raise DeployError(
            f"Folder contains {len(files)} files, exceeds limit of {config.MAX_FILE_COUNT}",
            code="QUOTA_EXCEEDED",
            status=413,
        )
    ...
    for upload_file in files:
        ...
        if first_path is None:
            first_path = relative_path

        normalized_path = _normalized_relative_path(relative_path)
        if os.path.isabs(relative_path) or relative_path.startswith("\\"):
            raise DeployError(f"Absolute path not allowed: {relative_path}")
        if ".." in normalized_path.parts:
            raise DeployError(f"Path traversal not allowed: {relative_path}")
        target_path = dest_dir / normalized_path
        if not _is_within_directory(dest_dir, target_path):
            raise DeployError(f"Unsafe path: {relative_path}")
        if _is_blocked(relative_path):
            raise DeployError(f"Blocked file type: {relative_path}")
        ...
        try:
            with target_path.open("wb") as output:
                while chunk := await upload_file.read(1024 * 1024):
                    ...
                    if total_size > config.MAX_TOTAL_SIZE or total_size > config.MAX_ZIP_SIZE:
                        raise DeployError(
                            "Total upload size exceeds configured limit",
                            code="QUOTA_EXCEEDED",
                            status=413,
                        )
                    output.write(chunk)
    ...
    return file_count, total_size, first_path
